...
fn take_digits(i: &[u8]) -> IResult<&[u8], u32> {
    let (i, digits) = take_while(AsChar::is_dec_digit)(i)?;
...
fn fraction_millisecond(i: &[u8]) -> IResult<&[u8], u32> {
    let (i, mut digits) = take_while(AsChar::is_dec_digit)(i)?;
